...
def make_parser():
    ...
    # Data
    parser.add_argument('--data_path', type=str, default='/media/hdd/mimic-cxr-eye/master_sheet.csv', help='Data path')
    parser.add_argument('--image_path', type=str, default='/media/hdd/mimic-cxr-jpg', help='image_path')
    parser.add_argument('--heatmaps_path', type=str, help='Heatmaps directory',
                        default='/media/hdd/mimic-cxr-eye/fixation-heatmaps')
    parser.add_argument('--output_dir', type=str, default='results', help='Output directory')
    parser.add_argument('--class_names', type=list, default=['Normal', 'CHF', 'pneumonia'],
                        help='Label names for classification')
...
def load_data(model_type, data_path, image_path, heatmaps_path, input_size, class_names, batch_size, num_workers, rseed,
              heatmaps_threshold, train_idx_path, test_idx_path):
    # ImageNet normalization
    mean = [0.485, 0.456, 0.406]
    std = [0.229, 0.224, 0.225]
    train_file = split_dataset(data_path, random_state=rseed)
    ...
    dataset = EyegazeDataset(train_file, image_path, class_names, static_heatmap_path=static_heatmap_path,
    ...
    train_dl = torch.utils.data.DataLoader(dataset, batch_size=batch_size, sampler=train_sampler,
                                           ...
                                           pin_memory=True,
                                           drop_last=True)
    test_dl = torch.utils.data.DataLoader(dataset, batch_size=batch_size, sampler=valid_sampler,
                                          num_workers=num_workers,
                                          pin_memory=True,
                                          drop_last=True)
...
if __name__ == '__main__':
    ...
    train_dl, valid_dl, test_dl = load_data(args.model_type, args.data_path, args.image_path, args.heatmaps_path, args.resize,
                                            args.class_names, args.batch_size, args.num_workers, args.rseed,
                                            args.heatmaps_threshold, args.train_idx_path, args.test_idx_path)
