...
def get_models(args, train=True, as_ensemble=False, model_file=None):
    ...
    for i in iter_m:
        if args.model == 'large-cnn':
            model = Net(dropout_prob1=args.dropout, dropout_prob2=args.dropout, num_classes=num_classes)
        elif args.model == 'small-cnn':
            model = SmallNet(dropout_prob1=args.dropout, num_classes=num_classes)
        elif args.model == 'mlp':
            model = MLP(num_features=num_features, dropout_prob=args.dropout, num_classes=num_classes)
        else:
...
def main():
    ...
    model_args.add_argument('--model-type', choices=['large-cnn', 'small-cnn', 'mlp'], default='large-cnn',
                            help='Model to train as ensemble sub-models (default: large-cnn)')
    ...
    data_args.add_argument('--dataset', choices=['mnist', 'femnist', 'leaf', 'compas', 'adult', 'texas', 'nature'],
    ...
    data_args.add_argument('--data-idx-path', type=str, default=None,
    ...
    # Make args compatible with femnist code
    ...
    if args.data_idx_path is not None:
        data_idx_file = args.data_idx_path.split('/')[-1]
    else:
