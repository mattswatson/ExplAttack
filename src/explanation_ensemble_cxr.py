...
from train_target_model import Net, SmallNet, MLP, get_dataset
from cxr import load_data
...
def get_models(args, train=True, as_ensemble=False, model_file=None):
    ...
    for i in iter_m:
        if args.model_type == 'densenet':
            ...
            raise NotImplementedError('Model architecture {} not supported'.format(args.model))
...
def get_shap_consistency(shap):
    # shap: batch_size x num_models x flat input
    shap = shap.numpy()
    mean = np.mean(shap, axis=1)
    std = np.std(shap, axis=1)
    cov = np.nan_to_num(stats.variation(shap, axis=1) * 100, nan=0)

    return 100 - (np.sum(cov) / np.multiply(*cov.shape))
...
def get_average_shap(models, loader, save_path, epoch, args):
    ...
    for batch_idx, (inputs, _, targets) in enumerate(loader):
...
def train(models, loader, epoch, criterion, optimizer, scheduler, writer, args, seeds=None, discrim=None,
    ...
    for batch_idx, (inputs, _, targets) in enumerate(loader):
        ...
        #consistency = get_shap_consistency(ensemble_shap)
        ...
        for i, m in enumerate(models):
            ...
            with torch.no_grad():
                ...
                submodel_correct[i] += pred.eq(targets.view_as(pred)).sum().item()

        output = ensemble(inputs)
        ensemble_loss = loss_fn(output, targets)
        ...
        if epoch % args.step == 1:
            discrim_out = discrim(all_outputs)
        else:
            discrim_out = discrim(all_outputs.detach())
        ...
        discrim_losses += args.beta * discrim_loss.item()
...
def test(models, loader, criterion, writer, epoch, verbose=False):
    ...
    total = 0
    with torch.no_grad():
        for _, (inputs, _, targets) in enumerate(loader):
            inputs, targets = inputs.float(), targets.long()
...
def main():
    ...
    model_args.add_argument('--model-type', choices=['densenet'], default='densenet',
                            help='Model to train as ensemble sub-models (default: densenet)')
    ...
    model_args.add_argument('--dropout', type=float, default=0.25, help='Dropout probability to use (default: 0.25)')
    ...
    parser.add_argument('--num-workers', type=int, default=1, help='Number of dataset workers to use if using CUDA')
    ...
    data_args.add_argument('--dataset', choices=['egd'], default='egd', help='Dataset to use')
    ...
    data_args.add_argument('--cxr-data-path', type=str, default='/media/hdd/mimic-cxr-jpg',
                              help='Path to JPG CXR data')
    data_args.add_argument('--gaze-data-path', type=str, default='/media/hdd/mimic-cxr-eye',
                              help='Path the EGD data')
    data_args.add_argument('--generated-heatmaps-path', type=str, default=None,
                              help='Path to pre-generated heatmaps. If None, generate heatmaps at runtime')
    data_args.add_argument('--label', choices=['Normal', 'CHF', 'Pneumothorax', 'all'], default='all',
                              help='Label to predict')
    data_args.add_argument('--train-idx-path', type=str, default=None)
    data_args.add_argument('--test-idx-path', type=str, default=None)
    ...
    # Only put the the filename in the TB name rather than the full path
    if args.train_idx_path is not None:
        data_idx_file = args.train_idx_path.split('/')[-1]
    else:
        data_idx_file = 'noHoldout'

    tb_name = 'TEST{}-expl_ensemble-seed_{:d}-beta{}-idx{}-{}'.format(args.dataset, args.seed, args.beta,
                                                                      data_idx_file,
    ...
    discriminator = Discriminator(input_size=num_classes, num_classes=len(models)).to(device)
